...
class Paypal::CapturesController < Paypal::PaypalController
  def create
    if paypal_order_params[:order_id].present?
      post = Post.find_by(paypal_order_params[:post_id])
      if post.present?
  ...
  def create_and_login_user_from_paypay(response_body)
    user = User.find_or_initialize_by(payer_object(response_body).to_h)
    if user.new_record?
  ...
  def payer_object(response_body)
    OpenStruct.new(
      ...
      payer_id: response_body[:payer][:payer_id]
    )
  end
